...
private:
	...
	void updV(const dblArray3_t& dv);
...
public:
	...
	double getq_m() { return qOVERm_m; }
	double getM() { return mass_m; }
	void updPmanual(const dblArray3_t& P) { position_m = P; }
	void updVmanual(const dblArray3_t& V) { velocity_m = V; }
	
	void updP(const dblArray3_t& F, double dt);
	void updP(const dblArray3_t& dF, double dt, bool newmethod);
	dblArray3_t calcBatP(const dblArray3_t& P, int norder = 0); //Mag field generated by moving charged particle
	//dblArray3_t totalFatP(BField* BFieldObj, this, double dt, int norder) { return nulldA3_t; }
};
